...
import { notifyServerEvent } from "@/lib/alerts/notify";
...
export async function POST(req: Request) {
  ...
  notifyServerEvent(
    { name, configJson: JSON.stringify(merged) },
    {
      title: `Server installed — ${name}`,
      message: `${name} (${tpl.name}) was created by ${user.email}.`,
      severity: "info",
    },
  );
